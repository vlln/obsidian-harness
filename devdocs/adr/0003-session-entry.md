...
|--------|---------|------|------|------------|
| 注册 .session 扩展名和自定义 view type | 在 plugin.ts 中注册 `registerExtensions` + `registerView` | **可行** | `registerExtensions(["session"], VIEW_TYPE)` + `ItemView` 子类，编译通过 | spike/0001-session-entry |
| 创建 .session 文件 | 命令面板 → "Create new .session file" → 验证 JSON 文件写入 vault | **可行** | `app.vault.create()` 创建文件，`app.workspace.getLeaf().openFile()` 打开，编译通过 | spike/0001-session-entry |
